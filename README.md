...
- 以 `sessions.started_at` 作为统计边界
- 一个会话的 token 会整体归属到该会话的起始日
- 每日图表、总览、模型分布、平台分布、API 端点分布、模型 × 端点联合视图、历史累计均使用同一边界
- `days=1 / 3 / 7 / 14 / 30 / 全部` 切换时，同一日期的 token 值不会因范围变化而变化

### 说明

- **每日 Token 消耗**：按会话起始日聚合
- **时段活跃度**：按会话起始小时分布
- **消息数 / 工具调用数**：来源于 `sessions.message_count` / `sessions.tool_call_count`，按会话起始日归属
- **历史累计**：在 `全部` 视图下与总 Token 一致

这种口径更适合“以会话为核心”的统计场景，能让图表、总览和各维度拆分保持自洽。
